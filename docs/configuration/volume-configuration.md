...
| `nodePublishSecretRef.name`     | The name of the Kubernetes Secret containing S3 credentials (`key_id`, `secret_access_key`) for this specific volume. Used when `authenticationSource` is `"secret"`.                | `"my-volume-credentials"`         | Conditionally Yes           |
