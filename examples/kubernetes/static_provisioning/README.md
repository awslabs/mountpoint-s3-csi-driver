...
- `multiple_pods_one_pv.yaml` - same as above, with multiple pods mounting the same persistent volume, in Deployment kind, that can be used to scale pods, using HPA or other method to create more replicas. This example also demonstrates the Pod Sharing feature in V2, where multiple workloads can share a single Mountpoint instance for better resource utilization. See [Pod Sharing](../../docs/FEATURES.md#pod-sharing) for more details.
