...
- `multiple_pods_one_pv.yaml` - same as above, with multiple pods mounting the same persistent volume, in Deployment kind, that can be used to scale pods, using HPA or other method to create more replicas. This example also demonstrates the Mountpoint Pod Sharing feature, where multiple workloads can share a single Mountpoint instance for better resource utilization. See [Mountpoint Pod Sharing](../../../docs/MOUNTPOINT_POD_SHARING.md) for more details.
- `caching.yaml` - shows how to configure mountpoint to use a cache directory. See [Caching](../../../docs/CACHING.md) and [Mountpoint documentation](https://github.com/awslabs/mountpoint-s3/blob/main/doc/CONFIGURATION.md#caching-configuration) for more details on caching options.
