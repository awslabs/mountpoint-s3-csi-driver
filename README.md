...
* **Graceful Pod Eviction** - The CSI driver ensures proper eviction order during pod termination. Mountpoint pods ignore SIGTERM signals and have a 10-minute termination grace period, remaining active until all workload pods using the volume have terminated. This prevents "Transport endpoint is not connected" errors. It is recommended to keep workload pod termination grace periods under 5 minutes for graceful shutdown.
