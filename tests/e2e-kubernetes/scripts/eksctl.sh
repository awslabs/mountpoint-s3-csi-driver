...
function eksctl_delete_cluster() {
  ...
  if [ -n "$VPC_ID" ]; then
    ...
    SECURITY_GROUP=$(aws ec2 describe-security-groups --region ${REGION} | jq -r --arg VPC_ID "$VPC_ID" '.["SecurityGroups"][] | select(.["VpcId"]==$VPC_ID) | .["GroupId"]')
...
function delete_eni() {
  ...
  while (( remaining_attemps-- > 0 ))
  do
      if $(aws ec2 delete-network-interface --network-interface-id ${ENI_ID} --region ${REGION}); then
        return
      fi
      sleep 30
  done
